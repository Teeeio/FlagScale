...
class MultiExpertAttention(nn.Module):
    ...
    def forward(
        ...
        # q: [B, T, num_kv_heads, num_groups, head_dim]

        # Step 7: Attention计算
        # q: [B, T, num_kv_heads, num_groups, head_dim] (T = total_seq_len)
        # k: [B, S, num_kv_heads, head_dim]
        # GQA attention: 每个kv_head服务num_groups个query heads
        logits = torch.einsum("BTKGH,BSKH->BKGTS", q.to(torch.float32), k.to(torch.float32))
        # logits: [B, num_kv_heads, num_groups, T, S]

        # Step 8: Attention mask
        if attn_mask is not None:
            big_neg = -2.3819763e38
            # attn_mask can be [B, S, T] or [B, 1, S, T]
            if attn_mask.dim() == 3:
                attn_mask_expanded = attn_mask[:, None, None, :, :]  # [B, 1, 1, S, T]
            elif attn_mask.dim() == 4:
                attn_mask_expanded = attn_mask[:, None, :, :, :]  # [B, 1, S, T] -> [B, 1, 1, S, T]
            else:
                raise ValueError(f"Unexpected attn_mask shape: {attn_mask.shape}")
        ...
        # probs: [B, num_kv_heads, num_groups, T, S]
        encoded = torch.einsum("BKGTS,BSKH->BTKGH", probs, v.to(dtype))
        # encoded: [B, T, num_kv_heads, num_groups, head_dim]

        # Step 11: 合并groups
        # encoded: [B, T, num_kv_heads, num_groups, head_dim]
        encoded = einops.rearrange(encoded, "B T K G H -> B T (K G) H")
        # encoded: [B, T, num_heads, head_dim]
